...
for url in urls:
    # 创建对应目录
    # os.makedirs(f"pages_0630/{category}", exist_ok=True)
    os.makedirs("pages_0630/pulse_mcp", exist_ok=True)
    ...
    while True:  # 无限循环，直到检测到页面为空
        ...
        try:
            ...
            print(f'正在访问pulse-mcp第 {page} 页: {link}')
            driver.get(link)

            time.sleep(3)

            html_content = driver.page_source
            # filename = f'pages_0630/{category}/page_{page}.html'
            filename = f'pages_0630/pulse_mcp/page_{page}.html'
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(html_content)

            print(f'保存完成: {filename}')

            # 假设html_content是包含服务器卡片的HTML字符串
            soup = BeautifulSoup(html_content, 'html.parser')

            # 查找所有服务器卡片（使用data-test-id属性定位）
            server_cards = soup.find_all('div', class_='flex', attrs={'data-test-id': lambda x: x and x.startswith('mcp-server-grid-')})

            # 如果当前页面没有找到任何服务器卡片，说明该 category 已结束，跳出循环
            if not server_cards:
                # print(f"[{category}] 第 {page} 页没有找到任何数据，退出该类别的爬取！")
                print(f"pulse-mcp 第 {page} 页没有找到任何数据，退出该类别的爬取！")
                break
            ...
            for card in server_cards:
                # print(f"正在处理卡片: {card.prettify()}")  # 打印当前卡片的 HTML 内容（可选）
                
                # 提取名称（位于<h3>标签内）
                name_tag = card.find('h3', class_='text-20')
                name = name_tag.text.strip() if name_tag else None
                print(f"名称: {name}")
                
                # 提取描述（位于class包含"text-15"的<p>标签内）
                desc_tag = card.find('p', class_='text-15')
...
# http://example.com/tillie
                ...
                # href = card.find('a', href=True)['href'] if card.find('a', href=True) else None
                # href = card.href.strip() if href else None
                # href = card.find_all('a').get('href')
                # print(f"card: {card.prettify()}")
                href = card.a['href']
                print(f"href: {href}")
                # 
                
                link_pulse_mcp = "https://www.pulsemcp.com/servers" + href if href else None
                print(f"链接: {link_pulse_mcp}")
                
                data.append({
                    'Server Name': name,
                    'Description': description,
                    'Link': link_pulse_mcp  # 完整URL
                })
            ...
            # excel_filename = f'pages_0630/{category}/smithery_servers_page_{page}.xlsx'
            excel_filename = f'pages_0630/pulse_mcp/pulse_mcp_page_{page}.xlsx'
            ...
            print(f'访问失败: pulse-mcp 第 {page} 页, 错误: {e}')
    
    # 当该 category 所有页面的数据都抓取完后，保存到 Excel
    if all_data:
        df = pd.DataFrame(all_data)
        # excel_filename = f'pages_0630/{category}/smithery_servers_{category}.xlsx'
        excel_filename = f'pages_0630/pulse_mcp/pulse_mcp_0630.xlsx'
        df.to_excel(excel_filename, index=False, engine='openpyxl')
        print(f"数据已成功保存到 {excel_filename}")
